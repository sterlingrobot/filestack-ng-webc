 ...
 */
const isEdgePolyfill: () => boolean = () => /Edge\/\d{2}/.test(window.navigator.userAgent) && 'ShadyDOM' in window;
...
const attachEdgeHijack = (tries: number) => {
  ...
  if (input && hasShadyEvents) {
    return (input as any).__shady_addEventListener('click', e => e.stopPropagation());
  }
